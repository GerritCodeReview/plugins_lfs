...
public class LfsPathsTest {
  // this tests the same functionality as it is being tested in
  // com.google.gerrit.httpd.plugins.LfsPluginServletTest however in plugin
  private static final String[] URL_PREFIXES = new String[] {"/", "/a/", "/p/", "/a/p/"};

  @Test
  public void lfsLocksRegexMatch() {
    Pattern p = Pattern.compile(LfsPaths.LFS_LOCKS_PATH_REGEX);

    matches(p, "locks/lock_id/unlock", "lock_id");
  }

  @Test
  public void lfsLocksRegexDoesntMatch() {
    Pattern p = Pattern.compile(LfsPaths.LFS_LOCKS_PATH_REGEX);

    doesNotMatch(p, "locks/unlock");
    doesNotMatch(p, "something/locks/lock_id/unlock");
    doesNotMatch(p, "locks/lock_id");
  }

  @Test
  public void lfsUrlRegexMatch() {
  ...
  @Test
  public void lfsUrlRegexDoesntMatch() {
    Pattern p = Pattern.compile(LfsPaths.URL_REGEX);
    doesNotMatch(p, "/foo");
  ...
  private void matches(Pattern p, String input, String group1) {
    Matcher m = p.matcher(input);
    assertThat(m.matches()).isTrue();
    assertThat(m.group(1)).isEqualTo(group1);
  }

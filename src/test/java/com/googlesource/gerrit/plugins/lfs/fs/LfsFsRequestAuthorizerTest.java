...
public class LfsFsRequestAuthorizerTest {
  ...
  public void testVerifyAgainstToken() throws Exception {
    Token token = auth.generateToken("o", zeroId(), 1);
    assertThat(auth.verifyAgainstToken(token.value, "o", zeroId())).isTrue();
  }

  @Test
  public void testVerifyAgainstDifferentOperation() throws Exception {
    Token token = auth.generateToken("o", zeroId(), 1);
    assertThat(auth.verifyAgainstToken(token.value, "p", zeroId())).isFalse();
  }

  @Test
  public void testVerifyAgainstDifferentObjectId() throws Exception {
    Token token = auth.generateToken("o", zeroId(), 1);
    assertThat(auth.verifyAgainstToken(token.value, "o",

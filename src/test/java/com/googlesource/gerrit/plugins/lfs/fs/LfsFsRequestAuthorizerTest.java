...
public class LfsFsRequestAuthorizerTest {
  private final LfsFsRequestAuthorizer auth =
      new LfsFsRequestAuthorizer(new LfsFsAuthTokenProcessor(new LfsCipher()));

  @Test
  public void testVerifyAgainstToken() throws Exception {
    AuthInfo info = auth.generateToken("o", zeroId(), 1);
    assertThat(auth.verifyAgainstToken(info.authToken, "o", zeroId()))
        .isTrue();
  }
  ...
  public void testVerifyAgainstDifferentOperation() throws Exception {
    AuthInfo info = auth.generateToken("o", zeroId(), 1);
    assertThat(auth.verifyAgainstToken(info.authToken, "p", zeroId()))
        .isFalse();
  }
  ...
  public void testVerifyAgainstDifferentObjectId() throws Exception {
    AuthInfo info = auth.generateToken("o", zeroId(), 1);
    assertThat(auth.verifyAgainstToken(info.authToken, "o",

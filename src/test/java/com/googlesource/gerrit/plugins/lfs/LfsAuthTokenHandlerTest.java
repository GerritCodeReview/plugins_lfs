...
public class LfsAuthTokenHandlerTest {
  ...
  public void testVerifyAgainstToken() throws Exception {
    Token token = auth.generateToken(1);
    assertThat(auth.verifyAgainstToken(token.value)).isTrue();
  }

  @Test
  public void testVerifyAgainstTokenWithParams() throws Exception {
    Token token = auth.generateToken(1, "s1", "s2");
    assertThat(auth.verifyAgainstToken(token.value, "s1", "s2")).isTrue();
  }

  @Test
  public void testVerifyAgainstTokenWithParamsInWrongOrder() throws Exception {
    Token token = auth.generateToken(1, "s1", "s2");
    assertThat(auth.verifyAgainstToken(token.value, "s2", "s1")).isFalse();
  }
  ...
  public void testVerifyAgainstInvalidToken() throws Exception {
    ...
    assertThat(auth.verifyAgainstToken(
        value.substring(1, 2) + value.substring(0, 1) + value.substring(2),
        "s")).isFalse();
  }

  @Test
  public void testVerifyAgainstDifferentParam() throws Exception {
    Token token = auth.generateToken(1, "s1");
    assertThat(auth.verifyAgainstToken(token.value, "s2")).isFalse();
  }
  ...
  public void testVerifyAgainstExpiredToken() throws Exception {
    ...
    Token token = auth.generateToken(-1);
    assertThat(auth.verifyAgainstToken(token.value)).isFalse();
  }
}

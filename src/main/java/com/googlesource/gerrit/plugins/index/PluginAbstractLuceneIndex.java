...
// TODO copy of com.google.gerrit.lucene.AbstractLuceneIndex<K, V>
// should be removed once corresponding class is accessible
/** Basic Lucene index implementation. */
public abstract class PluginAbstractLuceneIndex<K, V> implements Index<K, V> {
  private static final Logger log =
      LoggerFactory.getLogger(PluginAbstractLuceneIndex.class);

  protected static String sortFieldName(FieldDef<?, ?> f) {
  ...
  public static void setReady(SitePaths sitePaths, String name, int version,
    ...
    try {
      PluginGerritIndexStatus cfg = new PluginGerritIndexStatus(sitePaths);
  ...
  protected PluginAbstractLuceneIndex(
      Schema<V> schema,
      SitePaths sitePaths,
      Directory dir,
      String name,
      String subIndex,
      PluginGerritIndexWriterConfig writerConfig,
    ...
    if (commitPeriod < 0) {
      delegateWriter = new PluginAutoCommitWriter(dir, writerConfig.getLuceneConfig());
    } else if (commitPeriod == 0) {
      delegateWriter =
          new PluginAutoCommitWriter(dir, writerConfig.getLuceneConfig(), true);
    } else {
      final PluginAutoCommitWriter autoCommitWriter =
          new PluginAutoCommitWriter(dir, writerConfig.getLuceneConfig());
    ...
    searcherManager = new PluginWrappableSearcherManager(
  ...
  protected ListenableFuture<?> replace(final Term term, final Document doc) {
    return submit(new Callable<Long>() {
      @Override
      public Long call() throws IOException, InterruptedException {
  ...
  protected ListenableFuture<?> delete(final Term term) {
    return submit(new Callable<Long>() {
      @Override
      public Long call() throws IOException, InterruptedException {
  ...
  protected IndexSearcher acquire() throws IOException {
    return searcherManager.acquire();
  }

  protected void release(IndexSearcher searcher) throws IOException {
    searcherManager.release(searcher);
  }

  protected Document toDocument(V obj, FillArgs fillArgs) {
  ...
  protected void add(Document doc, Values<V> values) {

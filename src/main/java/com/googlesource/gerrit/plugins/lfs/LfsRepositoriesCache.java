...
public class LfsRepositoriesCache {
  private final LoadingCache<LfsBackendConfig, LargeFileRepository> cache;
  ...
  public LargeFileRepository get(LfsBackendConfig cfg) {
    return cache.getUnchecked(cfg);
  }

  public void put(LfsBackendConfig cfg, LargeFileRepository repo) {
    cache.put(cfg, repo);
  }

  static class Loader extends
    CacheLoader<LfsBackendConfig, LargeFileRepository> {
    ...
    @Override
    public LargeFileRepository load(LfsBackendConfig cfg)
        throws Exception {
      switch (cfg.type) {
        case FS:

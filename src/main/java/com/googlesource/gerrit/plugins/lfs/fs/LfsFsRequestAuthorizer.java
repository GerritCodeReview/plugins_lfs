...
public class LfsFsRequestAuthorizer {
  ...
  @Inject
  LfsFsRequestAuthorizer(FsProcessor processor) {
  ...
  public boolean verifyAuthInfo(String authToken, String operation,
    ...
    return new FsVerifier(token.get(), operation, id).verify();
  }

  static class FsProcessor
    extends LfsAuthToken.Processor<LfsFsAuthToken> {
    @Inject
    protected FsProcessor(LfsCipher cipher) {
      super(cipher);
    }
  ...
  private static class FsVerifier extends Verifier<LfsFsAuthToken> {
    private final String operation;
    private final AnyLongObjectId id;

    protected FsVerifier(LfsFsAuthToken token,

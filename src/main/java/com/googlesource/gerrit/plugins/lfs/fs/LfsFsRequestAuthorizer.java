...
public class LfsFsRequestAuthorizer {
  ...
  public AuthInfo generateAuthInfo(
      String operation, AnyLongObjectId id, Instant now, int expiresIn) {
    LfsFsAuthToken token = new LfsFsAuthToken(operation, id, now, expiresIn);
    return new AuthInfo(processor.serialize(token), token.expiresAt(), token.expiresIn);
  }
  ...
  private static class LfsFsAuthToken extends LfsAuthToken {
    ...
    LfsFsAuthToken(String operation, AnyLongObjectId id, Instant now, int expiresIn) {
      super(now, expiresIn);
      this.operation = operation;
      this.id = id;
    }

    LfsFsAuthToken(String operation, AnyLongObjectId id, String expiresAt, int expiresIn) {

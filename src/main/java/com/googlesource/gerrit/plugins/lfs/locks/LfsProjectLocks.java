...
class LfsProjectLocks {
  interface Factory {
    LfsProjectLocks create(String project);
  }
  ...
  @Inject
  LfsProjectLocks(
      PathToLockId toLockId, @PluginData Path defaultDataDir, @Assisted String project) {
    this.toLockId = toLockId;
    this.locksPath = Paths.get(defaultDataDir.toString(), "lfs_locks", project);
  ...
  LfsLock createLock(CurrentUser user, LfsCreateLockInput input) throws LfsException {
    ...
    try {
      ...
      if (!fileLock.commit()) {
        String error =
            String.format(
                "Committing locking to path [%s] in project %s failed", input.path, project);
        log.warn(error);
        throw new LfsException(error);
      }
    } finally {

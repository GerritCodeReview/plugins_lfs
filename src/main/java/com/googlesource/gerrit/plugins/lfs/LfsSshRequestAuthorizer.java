...
class LfsSshRequestAuthorizer {
  static class SshToken {
    private final LfsAuthTokenHandler.Token token;

    SshToken(LfsAuthTokenHandler.Token token) {
      this.token = token;
    }

    String getValue() {
      return SSH_AUTH_PREFIX + token.value;
    }
  ...
  SshToken generateToken(CurrentUser user, String project, String operation) {
    return new SshToken(auth.generateToken(expirationSeconds,
        user.getUserName(), project, operation));
  }

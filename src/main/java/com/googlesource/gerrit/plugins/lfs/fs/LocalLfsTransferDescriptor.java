...
public class LocalLfsTransferDescriptor extends FileLfsTransferDescriptor {
  ...
  protected FileLfsRepository getRepositoryFromPath(String path)
      throws IllegalArgumentException {
    int specDivider = path.lastIndexOf('/');
    String repoId = path.substring(0, specDivider);
    LocalLargeFileRepository repository = cache.getRepository(repoId);
    if (repository == null) {
      throw new IllegalArgumentException("There is no such FileLfsRepository");
    }

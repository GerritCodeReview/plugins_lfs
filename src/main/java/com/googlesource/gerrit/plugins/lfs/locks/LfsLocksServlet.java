...
public class LfsLocksServlet extends HttpServlet {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(LfsLocksServlet.class);
  static final DateTimeFormatter ISO = ISODateTimeFormat.dateTime();

  public static final String LFS_LOCKS_REST = String.format(
      URL_REGEX_TEMPLATE, LFS_LOCKS_PATH + "|" + LFS_VERIFICATION_PATH);
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (!matcher.matches()) {
      action.sendError(SC_INTERNAL_SERVER_ERROR,
          String.format("Unsupported path %s was provided", action.path));
      return;
    }
  ...
  private void verifyLocks(String project, Action action) throws IOException {
    ...
    action.sendResponse(new LfsVerifyLocksResponse(Collections.emptyList(),
  ...
  private void listLocks(String project, Action action) throws IOException {
    ...
    action.sendResponse(
        new LfsGetLocksResponse(Collections.emptyList(), input.cursor));
  }

  private void deleteLock(String project, String lockId, Action action)
      throws IOException {
    LfsDeleteLockInput input = action.input(LfsDeleteLockInput.class);
    log.debug("Delete (-f {}) lock for {} in project {}",
        Boolean.TRUE.equals(input.force), lockId, project);
    //TODO: this is just the method stub for lock deletion
    LfsLock lock = new LfsLock("random_id", "some/path/to/file", now(),
  ...
  private void createLock(String project, Action action) throws IOException {
    ...
    LfsLock lock = new LfsLock("random_id", input.path, now(),
        new LfsLockOwner("Lock Owner <lock_owner@example.com>"));
    action.sendResponse(lock);
  }
  ...
  private class Action {
    ...
    private Action(final HttpServletRequest req,
        final HttpServletResponse res) {
      this.path = req.getPathInfo().startsWith("/") ? req.getPathInfo()
          : "/" + req.getPathInfo();
      this.res = res;
